...
async function marc_extract()
{
	csv = await handle_input_file(document.getElementById("input_file2"))
	csv = csv_to_json(csv);
	console.log(csv)
	config.filter[get_param("issn")] = csv[0].values
	bundle_name = csv[0].name
	marc_data = await handle_input_file(document.getElementById("input_file1"))
	marc(marc_data);
}
